...
import ch.idsia.ai.agents.ai.GAgent;
...
public class AgentsPool
{
    ...
    public static void addAgent(Agent agent, String chromosome) {
        agentsHashMap.put(agent.getName(), agent);
    }

    public static void addAgent(String agentWOXName, String chromosome) throws IllegalFormatException
    {
        addAgent(load(agentWOXName, chromosome), chromosome);
    }

    public static Agent load (String name, String chromosome) {
        Agent agent;
        try {
            // agent = (Agent) Class.forName (name).newInstance (chromosome);
            agent = new GAgent(chromosome);
        }
