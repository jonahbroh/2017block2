...
int main(int argc, char** argv) {
  ...
  for(int i= 0; i < num_epochs; i++){
    ...
    printf("Generation %d, top fitness %d, %d msec, %s\n", i, pop.agents[pop_size - 1].fitness, msec, chromosome_string(pop.agents[pop_size - 1]));
  ...
  printf("Fitness: %d Generations: %d Chromosome: %s", pop.agents[pop_size - 1].fitness, 500, chromosome_string(pop.agents[pop_size - 1]));
  return 500;
}
